...
public class PlayerCharacterController : MonoBehaviour
{
	...
	void HandleCharacterMovement()
	{
		...
		bool isSprinting = m_InputHandler.GetSprintInputHeld();
		{
			...
			if (isGrounded)
			{
				...
				characterVelocity = Vector3.Lerp(characterVelocity, targetVelocity, movementSharpnessOnGround * Time.unscaledDeltaTime);
			...
			else
			{
				...
				characterVelocity += worldspaceMoveInput * accelerationSpeedInAir * Time.unscaledDeltaTime;
				...
				characterVelocity += Vector3.down * gravityDownForce * Time.unscaledDeltaTime;
			}
		}
		...
		m_Controller.Move(characterVelocity * Time.unscaledDeltaTime);

		// detect obstructions to adjust velocity accordingly
		m_LatestImpactSpeed = Vector3.zero;
		if (Physics.CapsuleCast(capsuleBottomBeforeMove, capsuleTopBeforeMove, m_Controller.radius, characterVelocity.normalized, out RaycastHit hit, characterVelocity.magnitude * Time.unscaledDeltaTime, -1, QueryTriggerInteraction.Ignore))
		{
	...
	void UpdateCharacterHeight(bool force)
	{
		...
		else if (m_Controller.height != m_TargetCharacterHeight)
		{
			...
			m_Controller.height = Mathf.Lerp(m_Controller.height, m_TargetCharacterHeight, crouchingSharpness * Time.unscaledDeltaTime);
			m_Controller.center = Vector3.up * m_Controller.height * 0.5f;
			playerCamera.transform.localPosition = Vector3.Lerp(playerCamera.transform.localPosition, Vector3.up * m_TargetCharacterHeight * cameraHeightRatio, crouchingSharpness * Time.unscaledDeltaTime);
		}
	}
